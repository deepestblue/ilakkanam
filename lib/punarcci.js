...
export const liftToArray = e => (! e) ? [] : Array.isArray(e,) ? e : [e,];

const anyமெய்Withஅ = anyOfArray(மெய்Withஅ,);
const anyவல்லினம்Withஅ = anyOfArray(வல்லினம்Withஅ,);
const VOWELS = anyOfIterable(vowelsToMarks.keys(),);

const RE_MONOSYLLABIC_SHORT_TERMINAL = RegExp(`^((?:${anyமெய்Withஅ}${anyOfArray(குறில்Markers,)})|(?:${anyமெய்Withஅ})|(?:${anyOfArray(குறில்Letters,)}))(${anyமெய்Withஅ})${புள்ளி}(${VOWELS})`, "gv",);
export const monosyllabicShortTerminalDoubler = வடிவு => வடிவு.replace(
...
const RE_TERMINAL_EE_OO = RegExp(`(${ஏMarker}|${ஓMarker}|${ஏLetter}|${ஓLetter})$`, "gv",);
export const terminalஏஓShortenerWithExtraந் = வடிவு => வடிவு.replace(
    RE_TERMINAL_EE_OO,
    (_unused, p1,) => `${LONG_TO_SHORT_ஏ_ஓ.get(p1,)}${ந}${புள்ளி}`,
);

const RE_LAST_AA_MARKER = RegExp(`${ஆMarker}([^${ஆMarker}]*)$`, "gv",);
...
const RE_PENULTIMATE_I_U_IMARKER_UMARKER = RegExp(`(${இLetter}|${இMarker}|${உLetter}|${உMarker})(.)${புள்ளி}`, "gv",);
...
export const புணர்ச்சிவிதிகள் = [
    s => s.replace(
        RegExp(`(${இLetter}|${இMarker}|${ஈLetter}|${ஈMarker}|${ஏLetter}|${ஏMarker})(${VOWELS})`, "gv",),
        (_unused, p1, p2,) => p1 + ய + புள்ளி + p2,
    ),
    s => s.replace(
        RegExp(`(${anyமெய்Withஅ}|${ஆMarker}|${ஆLetter}|${ஊMarker}|${ஊLetter}|${ஒMarker}|${ஒLetter}|${ஓMarker}|${ஓLetter})(${VOWELS})`, "gv",),
    ...
    s => s.replace(
        RegExp(`(?:${புள்ளி}|${உMarker})(${VOWELS})`, "gv",),
    ...
    s => s.replace(
        RegExp(`(${ன}|${ண})${புள்ளி}${anyவல்லினம்Withஅ}${புள்ளி}`, "gv",),
    ...
    s => s.replace(
        RegExp(`^${ஒLetter}(${ம}|${வ})(?!${புள்ளி})`, "gv",),
        (_unused, p1,) => ஒLetter + p1 + புள்ளி + p1,
    ),
];
