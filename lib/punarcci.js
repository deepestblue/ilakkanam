import { anyOfArray, anyOfIterable, } from "./utils.js";
import { vowelsToMarks, consonants, stopsToNasals, புள்ளி, அLetter, ஆLetter, ஆMarker, ஈLetterஇஈMarkers, உLetter, உMarker, ஊLetter, எMarker, ஏMarker, ஐLetter, ஐMarker, ஒLetter, ஒMarker, ஓLetter, ஓMarker, குறில், கசதப, த, ற, ட, ந, ன, ண, ப, ய, ல, வ, ள, } from "./ezuttu.js";

// Precomputed maps used in hot paths
const LIQUID_TO_STOP = new Map([[ல, ற,], [ள, ட,],],);
const LIQUID_TO_NASAL = new Map([[ல, ன,], [ள, ண,],],);
const NASAL_TO_STOP = new Map([[ன, ற,], [ண, ட,],],);
const LONG_TO_SHORT_VOWEL_MARKER = new Map([[ஏMarker, எMarker,], [ஓMarker, ஒMarker,],],);

// Precomputed regex fragments
const CONSONANTS = anyOfArray(consonants,);
const KURIL = anyOfArray(குறில்,);
const கசதப_FRAG = anyOfArray(கசதப,);
const VOWEL_KEYS = anyOfIterable(vowelsToMarks.keys(),);
const STOPS_VALUES = anyOfArray(Array.from(stopsToNasals.values(),),);
const STOPS_KEYS = anyOfArray(Array.from(stopsToNasals.keys(),),);

// Precompiled regexes
const RE_MONOSYLLABIC_SHORT_TERMINAL = RegExp(`^(${CONSONANTS}?${KURIL}?)(.)${புள்ளி}$`, "v",);
const RE_TERMINAL_LONG_EO = RegExp(`(${ஏMarker}|${ஓMarker})$`, "v",);
const RE_LAST_AA = RegExp(`${ஆMarker}([^${ஆMarker}]*)$`, "v",);
const RE_INITIAL_U = RegExp(`^${உLetter}`, "v",);
const RE_TERMINAL_L_STOPPER = RegExp(`(${ல})${புள்ளி}$`, "v",);
const RE_TERMINAL_V_U = RegExp(`${வ}${உMarker}$`, "v",);
const RE_TERMINAL_U = RegExp(`${உMarker}$`, "v",);
const RE_TERMINAL_THAI = RegExp(`(${த})$`, "v",);
const RE_LAST_STOP_DOUBLER = RegExp(`(?:(${STOPS_VALUES})${புள்ளி})?(${STOPS_KEYS})${உMarker}$`, "v",);

export const liquidToStop = liquid => LIQUID_TO_STOP.get(liquid,);
const liquidToNasal = liquid => LIQUID_TO_NASAL.get(liquid,);
const nasalToStop = nasal => NASAL_TO_STOP.get(nasal,);
const longToShortVowelMarker = vowelMarker => LONG_TO_SHORT_VOWEL_MARKER.get(vowelMarker,);

export const monosyllabicShortTerminalDoubler = வினய் => வினய்.replace(
    RE_MONOSYLLABIC_SHORT_TERMINAL,
    `$1$2${புள்ளி}$2${புள்ளி}`,
);

export const terminalஏஓShortener = வினய் => வினய்.replace(
    RE_TERMINAL_LONG_EO,
    (_unused, p1,) => longToShortVowelMarker(p1,),
);

export const lastஆShortener = வினய் => வினய்.replace(
    RE_LAST_AA,
    `$1`,
);

export const initialஉLengthener = வினய் => வினய்.replace(
    RE_INITIAL_U,
    `${ஊLetter}`,
);

export const terminalல்Stopper = வினய் => வினய்.replace(
    RE_TERMINAL_L_STOPPER,
    (_unused, p1,) => liquidToStop(p1,) + புள்ளி,
);

export const terminalவுRemover = வினய் => வினய்.replace(
    RE_TERMINAL_V_U,
    () => "",
);

export const terminalஉRemover = வினய் => வினய்.replace(
    RE_TERMINAL_U,
    () => "",
);

export const terminalதNasaliser = வினய் => வினய்.replace(
    RE_TERMINAL_THAI,
    (_unused, p1,) => stopsToNasals.get(p1,),
);

export const lastStopDoubler = வினய் => வினய்.replace(
    RE_LAST_STOP_DOUBLER,
    (_unused, _p1, p2,) => p2 + புள்ளி + p2 + உMarker,
);

// Cached புணர்ச்சி rules
const RE_PUNARCCI1 = RegExp(`${புள்ளி}(${VOWEL_KEYS})`, "gv",);
const RE_PUNARCCI2 = RegExp(`${உMarker}(${VOWEL_KEYS})`, "gv",);
const RE_PUNARCCI3 = RegExp(`(${anyOfArray(ஈLetterஇஈMarkers,)})(${VOWEL_KEYS})`, "gv",);
const RE_PUNARCCI4 = RegExp(`(${CONSONANTS}|${ஆMarker}|${ஆLetter}|${ஓMarker}|${ஓLetter})(${VOWEL_KEYS})`, "gv",);
const RE_PUNARCCI5 = RegExp(`(^${ஒLetter})(${VOWEL_KEYS})`, "gv",);
const RE_PUNARCCI6 = RegExp(`(${ல}|${ள})${புள்ளி}${கசதப_FRAG}${புள்ளி}`, "gv",);
const RE_PUNARCCI7 = RegExp(`^(${CONSONANTS}?${KURIL}?)(?:(${ல}|${ள})${புள்ளி}${த})`, "v",);
const RE_PUNARCCI8 = RegExp(`(${ல}|${ள})${புள்ளி}${த}`, "gv",);
const RE_PUNARCCI9 = RegExp(`(${ன}|${ண})${புள்ளி}${த}${புள்ளி}`, "gv",);
const RE_PUNARCCI10 = RegExp(`(${ல}|${ள})${புள்ளி}${ந}`, "gv",);
const RE_PUNARCCI11 = RegExp(`(${ற}|${ட})${புள்ளி}${த}`, "gv",);
const RE_PUNARCCI12 = RegExp(`(${ன}|${ண})${புள்ளி}${த}`, "gv",);
const RE_PUNARCCI13 = RegExp(`${ல}${புள்ளி}${ன}`, "gv",);
const RE_PUNARCCI14 = RegExp(`${ள}${புள்ளி}${ண}`, "gv",);
const RE_PUNARCCI15 = RegExp(`(${ன}|${ண})${புள்ளி}${வ}`, "gv",);
const புணர்ச்சிவிதி = [
    s => s.replace(
        RE_PUNARCCI1,
        (_unused, p1,) => vowelsToMarks.get(p1,),
    ),
    s => s.replace(
        RE_PUNARCCI2,
        (_unused, p1,) => vowelsToMarks.get(p1,),
    ),
    s => s.replace(
        RE_PUNARCCI3,
        (_unused, p1, p2,) => p1 + ய + vowelsToMarks.get(p2,),
    ),
    s => s.replace(
        RE_PUNARCCI4,
        (_unused, p1, p2,) => p1 + வ + vowelsToMarks.get(p2,),
    ),
    s => s.replace(
        RE_PUNARCCI5,
        (_unused, p1, p2,) => p1 + வ + புள்ளி + வ + vowelsToMarks.get(p2,),
    ),
    s => s.replace(
        RE_PUNARCCI6,
        (_unused, p1,) => liquidToStop(p1,) + புள்ளி,
    ),
    s => s.replace(
        RE_PUNARCCI7,
        (_unused, p1, p2,) => p1 + p2 + புள்ளி + p2 + புள்ளி + த,
    ),
    s => s.replace(
        RE_PUNARCCI8,
        (_unused, p1,) => p1 + உMarker + த,
    ),
    s => s.replace(
        RE_PUNARCCI9,
        (_unused, p1,) => nasalToStop(p1,) + புள்ளி,
    ),
    s => s.replace(
        RE_PUNARCCI10,
        (_unused, p1,) => liquidToNasal(p1,),
    ),
    s => s.replace(
        RE_PUNARCCI11,
        (_unused, p1,) => p1 + புள்ளி + p1,
    ),
    s => s.replace(
        RE_PUNARCCI12,
        (_unused, p1,) => p1 + புள்ளி + nasalToStop(p1,),
    ),
    s => s.replace(
        RE_PUNARCCI13,
        () => ன + புள்ளி + ன,
    ),
    s => s.replace(
        RE_PUNARCCI14,
        () => ண + புள்ளி + ண,
    ),
    s => s.replace(
        RE_PUNARCCI15,
        (_unused, p1,) => p1 + புள்ளி + ப,
    ),
];

// Conversion from modern spelling to traditional
const RE_OLD_AIKARAM1 = RegExp(`(${ஐLetter}|${ஐMarker})${ய}`, "gv",);
const RE_OLD_AIKARAM2 = RegExp(`(${ஐLetter}|${ஐMarker})`, "gv",);
export const அய்காரத்துப்பழயவிதி = [
    s => s.replace(
        RE_OLD_AIKARAM1,
        () => `${ய}`,
    ),
    s => s.replace(
        RE_OLD_AIKARAM2,
        () => `${ய}${புள்ளி}`,
    ),
];

// Conversion from traditional to modern spelling
const RE_AIKARAM1 = RegExp(`(${CONSONANTS})${ய}${புள்ளி}`, "gv",);
const RE_AIKARAM2 = RegExp(`(${CONSONANTS})${ய}`, "gv",);
const RE_AIKARAM3 = RegExp(`${அLetter}${ய}${புள்ளி}`, "gv",);
const RE_AIKARAM4 = RegExp(`${அLetter}${ய}`, "gv",);
const அய்காரத்துப்புதுவிதி = [
    s => s.replace(
        RE_AIKARAM1,
        (_unused, p1,) => `${p1}${ஐMarker}`,
    ),
    s => s.replace(
        RE_AIKARAM2,
        (_unused, p1,) => `${p1}${ஐMarker}${ய}`,
    ),
    s => s.replace(
        RE_AIKARAM3,
        () => `${ஐLetter}`,
    ),
    s => s.replace(
        RE_AIKARAM4,
        () => `${ஐLetter}${ய}`,
    ),
];

export const புணர்ச்சி = (புத்தெழுத்துமுறயோ,) => {
    if (! புத்தெழுத்துமுறயோ) {
        return புணர்ச்சிவிதி;
    }

    return புணர்ச்சிவிதி.concat(அய்காரத்துப்புதுவிதி,);
};
