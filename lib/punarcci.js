...
const மெய்WithஅFRAG = anyOfArray(மெய்Withஅ,);
const வல்லினம்WithஅFRAG = anyOfArray(வல்லினம்Withஅ,);
const VOWEL_KEYS = anyOfIterable(vowelsToMarks.keys(),);
const STOPS_TO_NASALS_VALUES = anyOfIterable(stopsToNasals.values(),);
const STOPS_TO_NASALS_KEYS = anyOfIterable(stopsToNasals.keys(),);
const AA_LETTER_OR_MARKER = anyOfArray([ஆLetter, ஆMarker,],);
const VOWEL_MARKERS = anyOfIterable(vowelMarkers,);

const RE_MONOSYLLABIC_SHORT_TERMINAL = RegExp(`^((?:${மெய்WithஅFRAG}${anyOfArray(குறில்Markers,)})|(?:${மெய்WithஅFRAG})|(?:${anyOfArray(குறில்Letters,)}))(${மெய்WithஅFRAG})${புள்ளி}(${VOWEL_KEYS})`, "v",);
...
const RE_LAST_STOP_DOUBLER = RegExp(`(?:(${STOPS_TO_NASALS_VALUES})${புள்ளி})?(${STOPS_TO_NASALS_KEYS})${உMarker}$`, "v",);
...
export const terminalஏஓShortenerWithExtraந் = பகுதி => பகுதி.replace(
    RE_TERMINAL_EE_OO,
    (_unused, p1,) => `${longToShortஏஓ(p1,)}ந்`,
);
...
export const terminalஆtoஎconverterWithExtraத் = பகுதி => பகுதி.replace(
    RE_TERMINAL_AA,
    (_unused, p1,) => (p1 === ஆMarker) ? `${எMarker}த்` : `${எLetter}த்`,
);
...
export const terminalவுtoபுconverter = பகுதி => பகுதி.replace(
    RE_TERMINAL_V_U,
    () => "பு",
);
...
const RE_PUNARCCI4 = RegExp(`(${மெய்WithஅFRAG}|${ஆMarker}|${ஆLetter}|${ஊMarker}|${ஊLetter}|${ஓMarker}|${ஓLetter})(${VOWEL_KEYS})`, "gv",);
const RE_PUNARCCI5 = RegExp(`(^${ஒLetter})(${VOWEL_KEYS})`, "gv",);
const RE_PUNARCCI6 = RegExp(`(${ல}|${ள})${புள்ளி}${வல்லினம்WithஅFRAG}${புள்ளி}`, "gv",);
...
const RE_PUNARCCI14 = RegExp(`${வ}(${வ}${மெய்WithஅFRAG}${புள்ளி})`, "gv",);
const RE_PUNARCCI15 = RegExp(`(${ல}|${ள})${புள்ளி}${ம}`, "gv",);
const RE_PUNARCCI16 = RegExp(`${அLetter}${புள்ளி}`, "gv",);
const RE_PUNARCCI17 = RegExp(`${அLetter}(${VOWEL_MARKERS})`, "gv",);
...
export const புணர்ச்சிவிதிகள் = [
    ...
    s => s.replace(
        RE_PUNARCCI15,
        (_unused, p1,) => liquidToNasal(p1,) + புள்ளி + ம,
    ),
    s => s.replace(
        RE_PUNARCCI16,
        () => "",
...
const RE_AIKARAM1 = RegExp(`(${மெய்WithஅFRAG})${ய}${புள்ளி}`, "gv",);
const RE_AIKARAM2 = RegExp(`(${letterOrMark}${மெய்WithஅFRAG})${ய}`, "gv",);
