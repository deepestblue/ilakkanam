...
import { vowelsToMarks, consonants, stopsToNasals, புள்ளி, அLetter, ஆLetter, ஆMarker, இLetter, இMarker, ஈLetter, ஈMarker, ஈLetterஇஈMarkers, உLetter, உMarker, ஊLetter, ஊMarker, எMarker, ஏMarker, ஐLetter, ஐMarker, ஒLetter, ஒMarker, ஓLetter, ஓMarker, குறில், கசதப, த, ற, ட, ந, ன, ண, ப, ய, ல, வ, ள, } from "./ezuttu.js";
...
// Precompiled regexes
const RE_MONOSYLLABIC_SHORT_TERMINAL = RegExp(`^(${CONSONANTS}?${KUTIL}?)(.)${புள்ளி}$`, "v",);
...
export const penultimateஇஉlengthener = வினய் => வினய்.replace(
...
const RE_PUNARCCI7 = RegExp(`^(${CONSONANTS}?${KUTIL}?)(?:(${ல}|${ள})${புள்ளி}${த})`, "v",);
...
const RE_PUNARCCI14 = RegExp(`(${ன}|${ண})${புள்ளி}${வ}`, "gv",);
const புணர்ச்சிவிதி = [
    s => s.replace(
        RE_PUNARCCI1,
