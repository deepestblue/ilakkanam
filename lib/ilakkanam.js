import { புணர்ச்சிவிதிகள்1, புணர்ச்சிவிதிகள்2, அய்காரத்துப்புதுவிதிகள், monosyllabicShortTerminalDoubler, } from "./punarcci.js";
import { வினய் as வினய்Class, } from "./vinay.js";
...
const treeTraverseAllLeaves = (node, callback, pathParams = [],) => {
    if (! node.children?.size) {
        callback(node, pathParams,);
        return;
    }

    node.children.forEach((childNode, childKey,) => treeTraverseAllLeaves(childNode, callback, [...pathParams, childKey,],),);
};

export const causativeFormsKey = "பிறவினய்வடிவு";

const buildவடிவுTree = (வினய், எழுத்துக்கூட்டுமுறய், seedGen,) => {
    const வினய்வடிவு = structuredClone(schema,);
    treeTraverseAllLeaves(வினய்வடிவு, (node, pathParams,) => {
        const [first, ...rest] = pathParams;

        if (first === "வினய்") {
            node.வடிவு = வினய்.பகுதி;
            return;
        }
        ...
        node.வடிவு = புணர்ச்சிவிதிகள்1.reduce(
            (intermediateForms, புணர்ச்சிவிதி,) => new Set(Array.from(intermediateForms,).map(புணர்ச்சிவிதி,),),
            வினய்[first](...rest,),
        );

        node.வடிவு = புணர்ச்சிவிதிகள்2.reduce(
            (intermediateForms, புணர்ச்சிவிதி,) => new Set(Array.from(intermediateForms,).map(புணர்ச்சிவிதி,),),
            seedGen(node.வடிவு,),
        );

        if (எழுத்துக்கூட்டுமுறய் === "புதியது") {
            node.வடிவு = அய்காரத்துப்புதுவிதிகள்.reduce(
                (intermediateForms, புணர்ச்சிவிதி,) => new Set(Array.from(intermediateForms,).map(புணர்ச்சிவிதி,),),
                node.வடிவு,
            );
        }
...
export const getForms = (வினய்ப்பெயர், இனத்துப்பெயர், எழுத்துக்கூட்டுமுறய் = "பழயது",) => {
    const வினய்toவடிவு = வினய் => buildவடிவுTree(வினய், எழுத்துக்கூட்டுமுறய், initialForms => new Set(Array.from(initialForms,).map(monosyllabicShortTerminalDoubler,),),);
...
export const getவடிவுEndings = இனத்துப்பெயர் =>
    buildவடிவுTree(new வினய்Class("", வினயினம்Byபெயர்(இனத்துப்பெயர்,), "பழயது", true,), "பழயது", initialForms => initialForms,);
