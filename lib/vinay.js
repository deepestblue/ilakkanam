...
export const எழுத்துமுறய்கள் = {
    பழயது: "பழயது",
    புதியது: "புதியது",
};

export class வினய் {
    constructor(பகுதி, இனம், எழுத்துமுறய், byPassCheckForஇனம்MatchingInவினய்Data,) {
        // Account for modern spelling first
        this.originalபகுதி = (எழுத்துமுறய் === எழுத்துமுறய்கள்.புதியது) ?
...
const startsWith = prefix => வினய்ப்பெயர் => {
    ...
    // For the case where the prefix ends in a மெய், again the Unicode encoding of the மெய் alongwith an imolicit vowel as a single codepoint needs to be worked around, so that the உயிர்மெய் codepoint only matches உயிர்மெய்s with the same implicit vowel and not other உயிர்மெய்s containing the same மெய்.
