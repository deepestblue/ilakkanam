...
import { அய்காரத்துப்பழயவிதிகள், liftToArray, } from "./punarcci.js";
import { புள்ளி, மெய்Withஅ, vowelsToMarks, } from "./ezuttu.js";
...
export class வினய் {
    constructor(பகுதி, இனம், எழுத்துக்கூட்டுமுறய், byPassCheckForஇனம்MatchingInவினய்Data,) {
        ...
        this.பகுதி = (எழுத்துக்கூட்டுமுறய் === "புதியது") ?
                அய்காரத்துப்பழயவிதிகள்.reduce((acc, val,) => val(acc,), பகுதி,)
            : பகுதி;

        const வினயினத்துData = வினய்Data.get(this.பகுதி,);

        ({ இனம்: this.வினயினம், பெயர்ச்சொல்முறய்: this.பெயர்ச்சொல்முறய், } = (() => {
            if (இனம்) {
                ...
                if (byPassCheckForஇனம்MatchingInவினய்Data) {
                    return { இனம், பெயர்ச்சொல்முறய்: undefined, };
                }

                // Check if the passed‐in இனம் matches.
                const e = liftToArray(வினயினத்துData,).find(item => item.இனம் === இனம்,);
                if (! e) {
                    throw new Error(`“${this.பகுதி}” எனும் பகுதி “${இனம்.இனத்துப்பெயர்}” எனும் வினயினத்திற்குச் செல்லாதது.`,);
                }

                return { இனம், பெயர்ச்சொல்முறய்: e.பெயர்ச்சொல், };
            }

            if (! வினயினத்துData) {
                throw new Error(`“${this.பகுதி}” எனும் பகுதி அறியாதது.`,);
            }

            if (Array.isArray(வினயினத்துData,)) {
                const பெயர்கள் = வினயினத்துData.map(இனத்துData => இனத்துData.இனம்.இனத்துப்பெயர்,);
                throw new Error(`“${this.பகுதி}” எனும் பகுதிக்கு பல வினயினங்கள் உள்ளன. ${பெயர்கள்.map(v => `“${v}”`,).join(", ",)} என்பவற்றுள் ஒன்றய்த் தேர்ந்தெடுக்க.`, {
                    cause: { code: "ambiguousVerbClass", classes: பெயர்கள், },
                },);
            }

            return { இனம்: வினயினத்துData.இனம், பெயர்ச்சொல்முறய்: வினயினத்துData.பெயர்ச்சொல், };
        })());
        ...
        return new Proxy(this, {
            get(target, prop,) {
                ...
                if (prop === "பெயர்ச்சொல்") {
                    return () => value.call(target.வினயினம், target.பகுதி, target.பெயர்ச்சொல்முறய்,);
                }
...
const startsWith = prefix => வினய்ப்பெயர் => {
    ...
    return வினய்ப்பெயர்.startsWith(prefix,) && ! [...vowelsToMarks.values(),].includes(வினய்ப்பெயர்.slice(prefix.length,).at(0,),);
};
...
export const இனத்தில்வினய்கள் = இனத்துப்பெயர் => {
    const பெயர்கள் = [...வினய்Data.entries(),].filter(([, வினய்Props,],) => liftToArray(வினய்Props,).some(entry => entry.இனம்.இனத்துப்பெயர் === இனத்துப்பெயர்,),).map(([பெயர்,],) => பெயர்,).sort((a, b,) => a.localeCompare(b, "ta",),);
